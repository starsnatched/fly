...
def state_msg(clr=20.0, alt=5.0, vy=0.0, collide=False):
    return json.dumps({"type": "state", "clearance": clr, "altitude": alt,
                       "velY": vy, "collision": collide, "speed": 3.0})
...
async def main():
    async with websockets.connect(f"ws://localhost:{PORT}/stream") as ws:
        ...
        async def epoch(label, secs, bad):
            ...
            t = await tele(ws)
            print(f"{label}: dopa={t['dopa']:+.3f} danHz={t['danHz']:.1f} "
        ...
        t = await tele(ws)
        print(f"wiped : edited={t['memEdited']} lo={t['memLo']:.3f} hi={t['memHi']:.3f}")
        t = await epoch("re-edit+4s", 4.0, bad=True)
        t2 = await epoch("re-edit+8s", 4.0, bad=False)
        print("OK" if t["memEdited"] > 0 or t2["memEdited"] > 0
